            ...
            if(!empty($config['settings']['per_page'])){
                ...
                if ($comments) {
                    ...
                    if(empty($_POST['ajax_pager']['load_more'])){
                        $response->addCommand(new HtmlCommand('[data-ajax_comment_pager="'.$entity->id().'"] .comments_ajax_pager_wrap', $build));
                        $response->addCommand(new AppendCommand('body', '<script>jQuery(\'html, body\').animate({ scrollTop: jQuery(\'[data-ajax_comment_pager="'.$entity->id().'"]\').offset().top-50 }, 500);</script>'));
                    } else {
                        $response->addCommand(new InvokeCommand('[data-ajax_comment_pager="'.$entity->id().'"] .comment_load_more_pager', 'removeClass', ['ajax-load_more']));
                        $response->addCommand(new HtmlCommand('[data-ajax_comment_pager="'.$entity->id().'"] .comment_load_more_pager', \Drupal::service('renderer')->render($build['pager'])));
                        // ---
                        unset($build['pager']);
                        $response->addCommand(new BeforeCommand('[data-ajax_comment_pager="'.$entity->id().'"] .comment_load_more_pager', \Drupal::service('renderer')->render($build)));                        
                    }
                }
            }
        }
