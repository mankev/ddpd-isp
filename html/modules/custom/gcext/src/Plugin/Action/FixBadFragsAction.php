...
namespace Drupal\gcext\Plugin\Action;
...
use Drupal\gcext_migrate\Util\ConfigUtil;
...
class FixBadFragsAction extends ViewsBulkOperationsActionBase implements ViewsBulkOperationsPreconfigurationInterface, PluginFormInterface {
  ...
  public function getOriginalFragments($entity) {
    ...
    if (empty($migration)) {
      \Drupal::logger('gcext')->notice('Migration not tagged: ' . $entity->id());
      return $fragments;
    }
    ...
    if (empty($mig_uuid)) {
      \Drupal::logger('gcext')->notice('Could not find entity in migration: ' . $entity->id());
      return $fragments;
    }
    ...
    if (empty($id)) {
      \Drupal::logger('gcext')->notice('Could not find source id: ' . $entity->id());
      return $fragments;
    }
  ...
  public function getOriginalFragmentsV2($entity) {
    $fragments = [];
    $uri = 'http://dv16.gcext.ca/api/v1/get-body/' . $entity->uuid();
    $headers = [
      'Accept' => 'application/json; charset=utf-8',
      'Content-Type' => 'application/json',
    ];
